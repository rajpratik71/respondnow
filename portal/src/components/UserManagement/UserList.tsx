...
export const UserList: React.FC = () => {
  ...
  // Check permissions
  const canCreate = UserPermissions.canCreate(userRoles);
  const canUpdate = UserPermissions.canUpdate(userRoles);
  const canDelete = UserPermissions.canDelete(userRoles);
      ...
      <div className={styles.card}>
        {!users || users.length === 0 ? (
          <div className={styles.emptyState}>
            ...
            <p>{canCreate ? 'Get started by creating your first user' : 'No users available to view'}</p>
            {canCreate && (
              <button className={styles.btnPrimary} onClick={() => setCreateDialogOpen(true)}>
                <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
                  <path d="M8 2a.5.5 0 0 1 .5.5v5h5a.5.5 0 0 1 0 1h-5v5a.5.5 0 0 1-1 0v-5h-5a.5.5 0 0 1 0-1h5v-5A.5.5 0 0 1 8 2z"/>
